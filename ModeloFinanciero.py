...
def fetch_market_data(tickers_list, period="5y"):
    """Descarga precios y volúmenes de Yahoo Finance con manejo de errores."""
    if not tickers_list:
        return None, None
    ...
    if missing:
        st.warning(f"Tickers no encontrados en Yahoo Finance: {', '.join(missing)}")
...
def calcular_retornos_robustos(prices, lookback_for_quantile=252):
    ...
    Usa rolling window en vez de expanding para evitar filtrar información futura.
    El parámetro lookback_for_quantile controla la ventana de estimación de cuantiles.
    """
    ...
    # Las primeras filas sin ventana suficiente quedan NaN; rellenar con retorno original
...
def generate_multi_factor_alpha(prices_df):
    """Genera señales alpha multi-factor (momentum, vol, trend, mean-reversion)."""
    if prices_df.empty:
        return pd.Series(0, index=prices_df.columns)
    returns = prices_df.pct_change().dropna()
    # Volatilidad semi-anualizada para escalar momentum de 6 meses consistentemente
    ...
    # FIX: escalar momentum por volatilidad en la misma frecuencia (126 días)
...
def generate_panel_ml_alpha(prices_df, returns_df, volumes_df):
    """Panel ML alpha: HistGradientBoosting sobre features cross-sectional.

    NO se cachea para evitar datos stale en ejecución live.
    """
    panel_data = []
...
def calculate_betas(returns_df, market_returns):
    ...
    for i in range(Y.shape[1]):
        try:
            ...
            betas.append(1.0)  # fallback conservador
...
def detect_regime_gmm(market_returns):
    ...
    # Probar 2 y 3 componentes, seleccionar por BIC
    best_bic = np.inf
    best_gmm = None
    for n in [2, 3]:
    ...
    # El estado con menor volatilidad media es el "safe" state
    vol_col_idx = 1  # columna 'vol'
    safe_state = np.argmin(means[:, vol_col_idx])
...
def probabilistic_sharpe_ratio(port_returns, rf_daily, benchmark_sr=0.0):
    """Calcula Deflated Sharpe Ratio y Probabilistic Sharpe Ratio.

    Todas las operaciones se hacen en frecuencia diaria para consistencia,
    y se anualiza al final.
    """
    ...
    # PSR: probabilidad de que el SR real > benchmark_sr
    ...
    # DSR: ajuste por skewness y kurtosis, todo en frecuencia diaria antes de anualizar
...
def optimizar_market_neutral_pro(expected_returns, cov_matrix, current_weights, betas,
    ...
    # FIX: usar Generator en vez de seed global para no repetir los mismos escenarios
    ...
    constraints = (
        # Dólar-neutral: pesos suman 0
        {'type': 'eq', 'fun': lambda w: np.sum(w)},
        # FIX: beta-neutral con tolerancia en vez de igualdad estricta
        {'type': 'ineq', 'fun': lambda w: 0.05 - abs(np.dot(w, betas))},
        # Apalancamiento bruto <= 1.5
    ...
    if not res.success:
        st.warning(f"Optimizador no convergió: {res.message}. Usando pesos actuales.")
...
def backtest_walk_forward_meta_model(prices, volumes, activos, rf_daily, initial_capital=1e6):
    ...
    # FIX: Meta-model entrena con features PER-ASSET (no promedios agregados).
    # Cada muestra es (n_assets, 2) aplanada a (n_assets * 2,) para Ridge.
    X_meta = []
    y_meta = []
    prev_stat = None
    prev_ml = None
    ...
    for i in range(252, len(dates) - rebalance_freq, rebalance_freq):
        ...
        # FIX: winsorizar SOLO con datos in-sample
        ...
        if prev_stat is not None and prev_ml is not None:
            # Retorno realizado del período anterior (por activo)
            realized_per_asset = train_returns[activos].iloc[-rebalance_freq:].sum()
            # Features: alpha stat y ml del período anterior, per-asset
            for asset_idx, asset in enumerate(activos):
        ...
        if len(X_meta) > len(activos) * 3:
            ...
            # Predecir per-asset usando las mismas 2 features
...
if menu == "Live Execution Desk":
    ...
    with st.expander("UNIVERSO DE ACTIVOS (Añadir mínimo 5)", expanded=True):
        with st.form("add_asset_form"):
            ...
            with c1:
                nuevo_ticker = st.text_input("Ticker (ej. WALMEX)")
            with c2:
                weight_actual = st.number_input("Peso Actual %", value=0.0, step=1.0) / 100
            with c3:
    ...
    if st.session_state['meta_model'] is not None:
        ...
        st.markdown("<span class='badge-warning'>Meta-Model No Disponible. Usando Fallback por Régimen. Corra el Backtest primero.</span>", unsafe_allow_html=True)
    ...
    if len(st.session_state['cartera']) > 4:
        ...
        if st.button("Generar Órdenes para Mañana (T+1)"):
            with st.spinner("Descargando mercado, ejecutando Machine Learning & Risk Optimization..."):
                ...
                if len(activos) < 5:
                    st.error("No hay suficientes activos válidos (mínimo 5) para ejecutar el modelo.")
                    st.stop()
                ...
                # FIX: usar meta-model con features per-asset (2 features por activo)
                if st.session_state['meta_model'] is not None:
                    curr_X = np.column_stack([
                ...
                st.markdown("### Métricas del Portafolio Objetivo")
                ...
                if regime == "CRISIS / VOLATILE":
                    st.markdown(
                        "<div class='alert-box'><b>CRISIS DETECTADA:</b> El mercado presenta inestabilidad sistémica. "
                        "El target de volatilidad ha sido reducido automáticamente para preservar el capital.</div>",
                        unsafe_allow_html=True
                    )
                ...
                st.markdown("### Tabla de Órdenes (T+1)")
                ...
                st.download_button(
                    label="Exportar Libro de Órdenes (CSV)",
                    data=csv,
                    file_name='execution_orders_T1.csv',
                    mime='text/csv'
                )
    ...
    if len(st.session_state['cartera']) > 4:
        ...
        if st.button("Ejecutar Motor Cuantitativo Completo (Heavy Compute)"):
            with st.spinner("Procesando Panel ML, entrenando Ridge Meta-Model Causal y simulando Costos Reales..."):
                ...
                if len(activos) < 5:
                    st.error("No hay suficientes activos válidos (mínimo 5).")
                    st.stop()
                ...
                if len(oos_ret) > 0:
                    ...
                    fig_oos.add_trace(go.Scatter(
                        ...
                        name="Ridge Ensemble L/S Strategy",
                        line=dict(color=COLOR_ACENTO, width=3)
                    ))
                    fig_oos.add_trace(go.Scatter(
                        x=equity_bench.index, y=equity_bench,
                        name="IPC Benchmark (^MXX)",
                        line=dict(color="#888888", width=2, dash='dot')
                    ))
                    fig_oos.update_layout(
                        title="Curva de Capital Real con MC Shuffle Overlay (Neto de Slippage, CVaR Penalty, Beta~0)",
                        plot_bgcolor=COLOR_FONDO, paper_bgcolor=COLOR_FONDO, font_color=COLOR_TEXTO
                    )
                    ...
                    fig_dd.update_layout(
                        title="Profundidad de Drawdown Histórico",
                        plot_bgcolor=COLOR_FONDO, paper_bgcolor=COLOR_FONDO, font_color=COLOR_TEXTO
                    )
                    ...
                    st.warning("Datos insuficientes para procesar el modelo matemático.")
    else:
